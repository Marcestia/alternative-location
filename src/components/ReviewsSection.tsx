...
export default function ReviewsSection({
  ...
  return (
    <section className="mx-auto mt-16 w-full max-w-6xl px-5 sm:px-6 lg:px-12">
      <div className="relative overflow-hidden rounded-[40px] border border-black/5 bg-white/90 p-8 shadow-[0_30px_60px_rgba(30,25,20,0.12)]">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-4">
          ...
          <button
            className="rounded-full bg-[color:var(--accent)] px-5 py-2 text-xs font-semibold text-white shadow-[0_18px_30px_rgba(216,111,63,0.25)]"
            ...
            <div className="mt-6">
              <div className="flex gap-4 overflow-x-auto pb-4 pr-4 snap-x snap-mandatory">
                {reviews.map((review) => (
                  <article
                    key={review.id}
                    className="min-w-[260px] max-w-[320px] snap-start rounded-[28px] border border-black/5 bg-white/80 p-6 shadow-[0_24px_40px_rgba(30,25,20,0.08)]"
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="rounded-full bg-[color:var(--surface-2)] px-2 py-1 text-xs font-semibold text-[color:var(--accent)]">
                          {review.rating}/5
                        </span>
                      </div>
                    </div>
      ...
      {open && (
        <div className="mt-8 rounded-[28px] border border-black/5 bg-white/80 p-6 shadow-[0_24px_40px_rgba(30,25,20,0.08)]">
          <h3 className="text-xl font-semibold">Laisser un avis</h3>
          <p className="mt-2 text-sm text-[color:var(--muted)]">
            Avis reserve aux clients avec facture payee. Entrez votre email de commande pour activer le formulaire.
          </p>

          <div className="mt-4 flex flex-wrap items-center gap-3">
            <input
              className="min-w-[220px] flex-1 rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm"
              name="email"
              type="email"
              value={email}
              onChange={(event) => {
          ...
          <form action={onSubmit} className="mt-4 grid gap-4 text-sm" onKeyDown={preventEnterSubmit}>
            <fieldset
              className={`grid gap-4 ${
                checkState === "eligible" ? "opacity-100" : "pointer-events-none opacity-45"
              }`}
