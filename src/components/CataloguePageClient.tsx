  ...
  return (
    <article className={`group flex h-full flex-col rounded-[32px] border bg-[linear-gradient(180deg,rgba(255,255,255,0.99),rgba(248,242,236,0.94))] p-4 shadow-[0_24px_50px_rgba(30,25,20,0.06)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_30px_60px_rgba(30,25,20,0.1)] sm:p-5 ${isSelected ? "border-emerald-500 ring-2 ring-emerald-100" : "border-black/5 hover:border-black/10"}`}>
      <div className="relative overflow-hidden rounded-[24px] bg-[color:var(--surface)]">
          <button type="button" onClick={() => onOpenDetails(item)} className="block h-44 w-full text-left sm:h-52" aria-label={`Voir le d\u00e9tail de ${item.name}`}>
...
export default function CataloguePageClient({
  ...
  useEffect(() => {
    ...
    const savedSelection = parseRequestedSelection(
      window.sessionStorage.getItem(CONTACT_REQUEST_SELECTION_STORAGE_KEY)
    );

    if (!savedSelection.length) return;

    const availableItems = [...sections.flatMap((section) => section.items), ...uncategorized];
    const nextState: Record<string, SelectedItemVM> = {};
  ...
  }, [requestMode, sections, uncategorized]);
  ...
  const requestSelection = useMemo<RequestedSelectionItem[]>(
    () =>
      selectedItems.map((item) => ({
        ...
        unitPriceCents: item.rentalPriceCents,
  ...
  useEffect(() => {
    ...
    if (requestSelection.length > 0) {
      window.sessionStorage.setItem(
        CONTACT_REQUEST_SELECTION_STORAGE_KEY,
        stringifyRequestedSelection(requestSelection)
      );
    } else {
  ...
  const addItem = (item: ItemVM) => {
    ...
    setSelected((prev) => {
      ...
      const nextQty = Math.min(existing.quantity + 1, stockLimit);
      ...
      <div className="rounded-[30px] border border-black/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(247,240,233,0.96))] p-5 shadow-[0_20px_44px_rgba(30,25,20,0.08)]">
        ...
        {requestMode && eventDateValue ? (
          <p className="mt-2 text-xs font-semibold leading-5 text-[color:var(--accent)]">
            Disponibilites verifiees pour le {new Date(`${eventDateValue}T12:00:00`).toLocaleDateString("fr-FR")}.
          </p>
        ) : null}
        ...
        {requestMode ? (
          <Link
            href="/#contact"
            className="mt-4 inline-flex w-full items-center justify-center rounded-full border border-black/10 bg-white px-4 py-3 text-xs font-semibold text-[color:var(--ink)] transition hover:-translate-y-0.5 hover:border-black/20"
          >
            Valider ma selection dans le formulaire
          </Link>
        ) : null}
      </div>
    </div>
